...
if(!class_exists('PMW_AjaxHelper')):
  class PMW_AjaxHelper extends PMW_AdminHelper{
    ...
    public function pmw_pixels_support_save(){
      $t_data = isset($_POST['data'])?$_POST['data']:"";
      parse_str($t_data, $data);
      $ajax_nonce = $this->get_post_data_sanitize($data, "pmw_ajax_nonce");
      if($this->admin_safe_ajax_call($ajax_nonce, 'pmw_ajax_nonce')){
        $fields = array(
          "user_support" => array(
            "name" => $this->get_post_data_sanitize($data, "name"),
            "email" => $this->get_post_data_sanitize($data, "email_id","","email"),
            "issue" => $this->get_post_data_sanitize($data, "issue")
          )
        );
    ...
    public function get_post_pmw_pixels_option_sanitize($data){
      //$pixels = array("facebook_pixel", "pinterest_pixel", "snapchat_pixel");
      return array(
        "user" => array(
          "email_id" => $this->get_post_data_sanitize($data, "email_id","","email")
        ),
        "facebook_pixel" => array(
          "pixel_id" => $this->get_post_data_sanitize($data, "facebook_pixel_id"),
          "is_enable" => $this->get_post_data_sanitize($data, "facebook_pixel_is_enable", false)
        ),
        "pinterest_pixel" => array(
          "pixel_id" => $this->get_post_data_sanitize($data, "pinterest_pixel_id"),
          "is_enable" => $this->get_post_data_sanitize($data, "pinterest_pixel_is_enable", false)
        ),
        "snapchat_pixel" => array(
          "pixel_id" => $this->get_post_data_sanitize($data, "snapchat_pixel_id"),
          "is_enable" => $this->get_post_data_sanitize($data, "snapchat_pixel_is_enable", false)
        ),
        "privecy_policy" => array(
          "is_theme_plugin_list" => $this->get_post_data_sanitize($data, "is_theme_plugin_list", 0),
          "privecy_policy" => 1
        )
      );
    }
    ...
    public function pmw_pixels_save(){
      ...
      $ajax_nonce = $this->get_post_data_sanitize($data, "pmw_ajax_nonce");
      if($this->admin_safe_ajax_call($ajax_nonce, 'pmw_ajax_nonce')){
        $pixels_option = $this->get_post_pmw_pixels_option_sanitize($data);
